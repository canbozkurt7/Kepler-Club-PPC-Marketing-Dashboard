...
from ..fetchers.microsoft_ads import fetch_microsoft_ads_metrics
...
class PlatformSyncer:
    ...
    def sync_microsoft_ads(self, days: int = 1) -> bool:
        """Fetch and store Microsoft Ads (Bing Ads) data. No-op without credentials."""
        if not os.getenv("MICROSOFT_DEVELOPER_TOKEN") or not os.getenv("MICROSOFT_CUSTOMER_ID"):
            logger.warning("Skipping Microsoft Ads sync — MICROSOFT_DEVELOPER_TOKEN or MICROSOFT_CUSTOMER_ID not set")
            return False

        sync_log = SyncLog(
            platform_id=4,  # microsoft
            sync_type="microsoft_ads",
            sync_status="RUNNING",
            started_at=datetime.utcnow(),
        )
        self.db.add(sync_log)
        ...
        try:
            ...
            raw_data = fetch_microsoft_ads_metrics(date_from=date_from, date_to=date_to)
            normalized = self.normalizer.normalize_metrics(raw_data, "microsoft")
            enriched = self.enricher.enrich_records(normalized)
            segmented = CampaignSegmenter.segment_records(enriched)
            records_stored = self._store_metrics(segmented, "microsoft")
            ...
            logger.info(f"Microsoft Ads sync completed: {records_stored} records")
            return True
            ...
            sync_log.error_message = str(e)
            sync_log.completed_at = datetime.utcnow()
            logger.error(f"Microsoft Ads sync failed: {str(e)}")
            return False
...
def run_hourly_sync(days: int = 1):
    ...
    try:
        ...
        syncer.sync_microsoft_ads(days=days)
